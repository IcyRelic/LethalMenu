...
{
    private static readonly Hack[] KeyBindIgnore =
    {
        Hack.ModifyQuota,
        Hack.ModifyCredits,
        Hack.KillPlayer,
    ...
    private static readonly List<Hack> Waiting = [];
    ...
    private static readonly Dictionary<Hack, Delegate> Executors = new()
    {
    ...
    {
        return KeyBinds.GetValueOrDefault(hack);
    }
    ...
    {
        return KeyBinds.Keys.Any(hack => KeyBinds[hack] == btn);
    }
}
...
public class HackExecutor
{
    ...
    public static void UnlockDoor()
    {
        ...
        RaycastHit hitInfo;
        if (Physics.Raycast(
                new Ray(CameraManager.ActiveCamera.transform.position, CameraManager.ActiveCamera.transform.forward),
                out hitInfo, 5f, LayerMask.GetMask("InteractableObject")))
        {
        ...
        else
        {
                     ...
                     select obj)
            {
                ...
                HUDManager.Instance.DisplayTip("Lethal Menu",
                    type + " ( " + obj.objectCode + " ) has been called from the terminal.");
            }
        }
    }
    ...
    public static void ModExperience(int amt, ActionType type)
    {
        ...
        {
            ActionType.Add => HUDManager.Instance.localPlayerXP + amt,
            ActionType.Remove => HUDManager.Instance.localPlayerXP - amt,
            _ => amt
        };
    ...
    {
        HUDManager.Instance.DisplayTip("Lethal Menu", died.playerUsername + " has died from " + cause);
    }
    ...
    public static void UnlockUnlockable(Unlockable unlockable)
    {
        RoundHandler.BuyUnlockable(unlockable);
    }
