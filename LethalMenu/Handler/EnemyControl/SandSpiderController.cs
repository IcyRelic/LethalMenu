...
namespace LethalMenu.Handler.EnemyControl
{
    internal class SandSpiderController : IEnemyController<SandSpiderAI>
    {
        ...
        void PlaceWebTrap(SandSpiderAI enemy)
        {
            ...
            if (!Physics.Raycast(ray, out RaycastHit raycastHit, 7.0f, StartOfRound.Instance.collidersAndRoomMask))
            {
                return;
            }
            ...
            if (!Physics.Raycast(enemy.abdomen.position, Vector3.down, out RaycastHit groundHit, 10.0f, StartOfRound.Instance.collidersAndRoomMask))
            {
                return;
            }
