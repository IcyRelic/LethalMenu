...
namespace LethalMenu.Cheats
{
    ...
    internal class NoCooldownInteractions : Cheat
    {
        ...
        private IEnumerator TimeToHold()
        {
            while (true)
            {
                ...
                LethalMenu.interactTriggers.FindAll(t => t != null).ForEach(t =>
                {
                    ...
                    t.timeToHold = Hack.InstantInteract.IsEnabled() ? t.name.StartsWith("EntranceTeleport") ? 0.3f : 0.0f : original;
                });
            }
        }
        ...
        [HarmonyPatch(typeof(GrabbableObject), "RequireCooldown"), HarmonyPrefix]
        ...
        [HarmonyPatch(typeof(InteractTrigger), "Interact"), HarmonyPrefix]
        public static bool Interact(InteractTrigger __instance)
        {
            if (__instance.GetComponentInParent<DoorLock>() != null)
            {
                __instance.interactCooldown = true;
                DoorLock door = __instance.GetComponentInParent<DoorLock>();
                if (door.Reflect().GetValue<bool>("isDoorOpened")) __instance.timeToHold = 0.0f;
        ...
        [HarmonyPatch(typeof(StartMatchLever), nameof(StartMatchLever.BeginHoldingInteractOnLever)), HarmonyPostfix]
        public static void BeginHoldPostfix(StartMatchLever __instance)
        {
            ...
            if (triggers.TryGetValue(__instance.triggerScript.GetInstanceID(), out float holdTime) && holdTime != normal) triggers[__instance.triggerScript.GetInstanceID()] = normal;
            if (Hack.InstantInteract.IsEnabled()) __instance.triggerScript.timeToHold = 0.0f;
        }

        [HarmonyPatch(typeof(DoorLock), nameof(DoorLock.UnlockDoor)), HarmonyPostfix]
        public static void UnlockDoorPostfix(StartMatchLever __instance)
        {
            if (triggers.TryGetValue(__instance.triggerScript.GetInstanceID(), out float holdTime) && holdTime != 0.3f) triggers[__instance.triggerScript.GetInstanceID()] = 0.3f;
            if (Hack.InstantInteract.IsEnabled()) __instance.triggerScript.timeToHold = 0.0f;
        }

        [HarmonyPatch(typeof(Shovel), "reelUpShovel"), HarmonyPostfix]
