...
namespace LethalMenu.Cheats
{
    ...
    internal class Shoplifter : Cheat
    {
        ...
        public static int[] vehicleCosts = Array.Empty<int>();
        public static int[] itemCosts = Array.Empty<int>();
        ...
        public override void Update()
        {
            ...
            if(Hack.Shoplifter.IsEnabled())
            {
                ...
                if (itemCosts is null)
                {
                    ...
                    for (int i = 0; i < LethalMenu.terminal.buyableItemsList.Length; i++) itemCosts[i] = LethalMenu.terminal.buyableItemsList[i].creditsWorth;
                }
                if (vehicleCosts is null)
                {
                    vehicleCosts = new int[LethalMenu.terminal.buyableVehicles.Length];
                    for (int i = 0; i < LethalMenu.terminal.buyableVehicles.Length; i++) vehicleCosts[i] = LethalMenu.terminal.buyableVehicles[i].creditsWorth;
                }
            ...
            else if(nodeCosts.Count > 0)
            {
                ...
                for (int i = 0; i < LethalMenu.terminal.buyableItemsList.Length; i++) LethalMenu.terminal.buyableItemsList[i].creditsWorth = itemCosts[i];
                for (int i = 0; i < LethalMenu.terminal.buyableVehicles.Length; i++) LethalMenu.terminal.buyableVehicles[i].creditsWorth = vehicleCosts[i];
                Clear();
            }         
        }
        ...
        {
            nodeCosts.Clear();
            itemCosts = Array.Empty<int>();
            vehicleCosts = Array.Empty<int>();
        }

        [HarmonyPatch(typeof(Terminal), "LoadNewNodeIfAffordable"), HarmonyPrefix]
        public static void LoadNode(TerminalNode node)
        {   
            if (Hack.Shoplifter.IsEnabled()) node.itemCost = 0;
        }
    }       
}
