...
internal class SelfTab : MenuTab
{
    ...
    private void SelfHackContent()
    {
        ...
        UI.HackSlider(Hack.SuperJump, "SelfTab.SuperJump", Settings.f_jumpForce.ToString("0.0"),
            ref Settings.f_jumpForce, Settings.f_defaultJumpForce, Settings.f_defaultJumpForce + 200);
        UI.HackSlider(Hack.FastClimb, "SelfTab.FastClimb", Settings.f_climbSpeed.ToString("0.0"),
            ref Settings.f_climbSpeed, Settings.f_defaultClimbSpeed, Settings.f_defaultClimbSpeed + 20);
        UI.HackSlider(Hack.SuperSpeed, "SelfTab.SuperSpeed", Settings.f_movementSpeed.ToString("0.0"),
            ref Settings.f_movementSpeed, Settings.f_defaultMovementSpeed, Settings.f_defaultMovementSpeed + 20);
        UI.HackSlider(Hack.NoClip, "SelfTab.NoClip", Settings.f_noclipSpeed.ToString("0.0"), ref Settings.f_noclipSpeed,
            10f, 30f);
    ...
    private void ExperienceContent()
    {
        ...
        var level = (bool)HUDManager.Instance ? HUDManager.Instance.localPlayerLevel : 0;
        var xp = (bool)HUDManager.Instance ? HUDManager.Instance.localPlayerXP : 0;
    ...
    private void DoorTeleportLocations(PlayerControllerB player, List<EntranceTeleport> doors)
    {
        ...
        {
            var textKey = door.isEntranceToBuilding ? "SelfTab.TeleportEntrance" : "SelfTab.TeleportExit";
            var localizedText = Localization.Localize(textKey);

            UI.Hack(Hack.Teleport, $"{localizedText} {c}", door.entrancePoint.position, false, true,
                door.isEntranceToBuilding);
            c++;
        }
    }
}